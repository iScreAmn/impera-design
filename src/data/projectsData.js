import {
  antonovAndCo1,
  antonovAndCo2,
  antonovAndCo3,
  antonovAndCo4,
  antonovAndCo5,
  antonovAndCo6,
  antonovAndCo7,
  antonovAndCo8,
  antonovAndCo9,
  antonovAndCo10,
  antonovAndCo11,
  antonovAndCo12,
  antonovAndCo13,
  antonovAndCo14,
  antonovAndCo15,
  antonovAndCo16,
  antonovAndCo17,
  antonovAndCo18,
  antonovAndCo19,
  antonovAndCo20,
  antonovAndCo21,
  antonovAndCo22,
  antonovAndCo23,
  trophi1,
  trophi2,
  trophi3,
  trophi5,
  trophi6,
  trophi7,
  trophi8,
  trophi9,
  trophi11,
  flowersShop1,
  flowersShop2,
  flowersShop3,
  flowersShop4,
  flowersShop5,
  flowersShop6,
  flowersShop7,
  flowersShop8,
  flowersShop9,
  flowersShop10,
  bossLounge1,
  bossLounge2,
  bossLounge3,
  bossLounge4,
  bossLounge5,
  bossLounge6,
  bossLounge7,
  bossLounge8,
  bossLounge9,
  bossLounge10,
  bossLounge11,
  bossLounge12,
  bossLounge13,
  bossLounge14,
  bossLounge15,
  bossLounge16,
  lesnoiPark2,
  lesnoiPark3,
  lesnoiPark4,
  lesnoiPark5,
  lesnoiPark6,
  lesnoiPark7,
  lesnoiPark8,
  lesnoiPark9,
  lesnoiPark10,
  lesnoiPark11,
  lesnoiPark12,
  lesnoiPark13,
  lesnoiPark14,
  lesnoiPark15,
  lesnoiPark16,
  lesnoiPark17,
  lesnoiPark18,
  lesnoiPark19,
  lesnoiPark20,
  lesnoiPark21,
  taiMassage1,
  taiMassage2,
  taiMassage3,
  taiMassage4,
  taiMassage5,
  taiMassage6,
  taiMassage7,
  taiMassage8,
  taiMassage9,
  taiMassage10,
  taiMassage11,
  taiMassage12,
  taiMassage13,
  taiMassage14,
  taiMassage15,
  taiMassage16,
  taiMassage17,
  kidsRoom1,
  kidsRoom2,
  kidsRoom3,
  kidsRoom4,
  kidsRoom5,
  kidsRoom6,
  kidsRoom7,
  kidsRoom8,
  slowGlow1,
  slowGlow2,
  slowGlow3,
  slowGlow4,
  slowGlow5,
  slowGlow6,
  slowGlow7,
  slowGlow8,
  bulvar1,
  bulvar2,
  bulvar3,
  bulvar4,
  bulvar5,
  bulvar6,
  bulvar7,
  bulvar8,
  bulvar9,
  bulvar10,
  bulvar11,
  bulvar12,
  bulvar13,
  office1,
  office2,
  office3,
  office4,
  office5,
  office6,
  office7,
  office8,
  office9,
  office10,
  office11,
  office12,
  office13,
  office14,
  office15,
  office16,
  office17,
  officeNext1,
  officeNext2,
  officeNext3,
  officeNext4,
  officeNext5,
  officeNext6,
  officeNext7,
  officeNext8,
  officeNext9,
  officeNext10,
  officeNext11,
  officeNext12,
  officeNext13,
  officeNext14,
  officeNext15,
  officeNext16,
  officeNext17,
  officeNext18,
  officeNext19,
  officeNext20,
  officeNext21,
  officeNext22,
  officeNext23,
  officeNext24,
  officeNext25,
  officeNext26,
  officeNext27,
  officeNext28,
  officeNext29,
  officeNext30,
  dvorik1,
  dvorik2,
  dvorik3,
  dvorik4,
  dvorik5,
  dvorik6,
  dvorik7,
  dvorik8,
  dvorik9,
  hotel1,
  hotel2,
  hotel3,
  hotel4,
  hotel5,
  hotel6,
  hotel7,
  hotel8,
  hotel9,
  hotel10,
  hotel11,
  hotel12,
  hotel13,
  hotel14,
  hotel15,
  hotel16,
  hotel17,
  hotel18,
  hotel19,
  hotel20,
  hotel21,
  hotel22,
  hotel23,
  hotel24,
  hotel25,
  hotel26,
  hotelLbn1,
  hotelLbn2,
  hotelLbn3,
  hotelLbn4,
  hotelLbn5,
  hotelLbn7,
  hotelLbn8,
  hotelLbn9,
  hotelLbn10,
  hotelLbn11,
  hotelLbn12,
  hotelLbn13,
  hotelLbn14,
  hotelLbn15,
  hotelLbn16,
  hotelLbn17,
  hotelLbn18,
  hotelLbn19,
  hotelLbn20,
  hotelLbn21,
  hotelLbn22,
  hotelLbn23,
  restaurant1,
  restaurant2,
  restaurant3,
  restaurant4,
  restaurant5,
  restaurant6,
  restaurant7,
  restaurant8,
  restaurant9,
  restaurant10,
  restaurant11,
  paratun1,
  paratun2,
  paratun3,
  paratun4,
  paratun5,
  paratun6,
  paratun7,
  paratun8,
  paratun9,
  paratun10,
  paratun11,
  paratun12,
  paratun13,
  crazyBrothers1,
  crazyBrothers2,
  crazyBrothers3,
  crazyBrothers4,
  crazyBrothers5,
  crazyBrothers6,
  crazyBrothers7,
  crazyBrothers8,
  crazyBrothers9,
  crazyBrothers10,
  crazyBrothers11,
  crazyBrothers12,
  beautySalon1,
  beautySalon2,
  beautySalon3,
  beautySalon4,
  beautySalon5,
  beautySalon6,
  beautySalon7,
  beautySalon8,
  beautySalon9,
  beautySalon10,
  beautySalon11,
  beautySalon12,
  beautySalon13,
  beautySalon14,
  beautySalon15,
  beautySalon16,
  beautySalon17,
  beautySalon18,
  beautySalon19,
  beautySalon20,
  beautySalon21,
  dental1,
  dental2,
  dental3,
  dental4,
  dental5,
  dental6,
  dental7,
  dental8,
  dental9,
  dental10,
  dental11,
  dental12,
  dental13,
  dental14,
  dental15,
} from '../assets/images';

export const projectsPageData = {
  title: 'Проекты студии «Импера»',
  subtitle: 'Дизайн, который усиливает бренды: кафе и рестораны, бары и клубы, офисы, спа-комплексы, магазины, офисы. Чёткая концепция, продуманная эргономика и премиальный опыт для Ваших гостей и команды',
  homeTitle: 'Проекты студии «Импера»',
  buttons: {
    discuss: 'Обсудить проект',
    viewCases: 'Смотреть кейсы'
  }
};

export const projectsData = [
  {
    id: 1,
    slug: 'boss-lounge',
    title: 'Кальянная \n«BOSS-LOUNGE»',
    location: 'г. Москва',
    date: '2024 г.',
    area: '230 м²',
    studio: 'Impera Studio',
    description: 'Атмосферная, стильная, продуманная до мелочей кальянная - мечта нашего заказчика, которую мы воплотили в реальность. Каждая зона - это отдельный визуальный акцент, но с общим стилем и концепцией. Здесь роскошь и характер заведения ощущаются с первого взгляда.',
    features: [
      'Панорамные окна с видом на город',
      'Система умного освещения',
      'Премиальные отделочные материалы',
      'Индивидуальная мебель по проекту',
    ],
    images: [bossLounge1, bossLounge2, bossLounge3, bossLounge4, bossLounge5, bossLounge6, bossLounge7, bossLounge8, bossLounge9, bossLounge10, bossLounge11, bossLounge12, bossLounge13, bossLounge14, bossLounge15, bossLounge16],
    thumbnails: [bossLounge1, bossLounge2, bossLounge3, bossLounge4],
    video: {
      title: 'Реализация проекта',
      sources: [
        { src: '/videos/boss.mp4', type: 'video/mp4' },
        { src: '/videos/boss.webm', type: 'video/webm' }
      ],
      poster: bossLounge1
    }
  },
  {
    id: 2,
    slug: 'lesnoi-park',
    title: 'Спа-комплекс «Лесной»',
    location: 'г. Солнечногорск',
    date: '2025 г.',
    area: '700 м²',
    studio: 'Impera Studio',
    description: 'Мы превратили обычный оздоровительный центр в полноценный спа-комплекс. Стильный, с акцентом на домашний уют в современном, премиальном прочтении. Мягкая и спокойная палитра подчеркивает тишину и приватность. Свет, текстиль, натуральные оттенки - всё работает на расслабление.',
    features: [
      'Экологичные материалы',
      'Встроенные системы хранения',
      'Зонирование пространства',
      'Индивидуальное освещение в каждой зоне',
    ],
    images: [lesnoiPark20, lesnoiPark2, lesnoiPark3, lesnoiPark21, lesnoiPark4, lesnoiPark5, lesnoiPark6, lesnoiPark7, lesnoiPark8, lesnoiPark9, lesnoiPark10, lesnoiPark11, lesnoiPark12, lesnoiPark13, lesnoiPark14, lesnoiPark15, lesnoiPark16, lesnoiPark17, lesnoiPark18, lesnoiPark19, ],
    thumbnails: [lesnoiPark2, lesnoiPark3, lesnoiPark4],
  },
  {
    id: 3,
    slug: 'office-2',
    title: 'Офисный центр',
    location: 'г. Москва',
    date: '2026 г.',
    area: '430 м²',
    studio: 'Impera Studio',
    type: 'Для бизнеса',
    description: 'Современное офисное пространство, в котором сочетаются эстетика и функциональность. Лофт с кирпичными стенами, глубокими оттенками и лаконичной мебелью создаёт атмосферу, которая вдохновляет, привлекает и работает на результат.',
    features: [],
    images: [officeNext1, officeNext2, officeNext3, officeNext4, officeNext5, officeNext6, officeNext7, officeNext8, officeNext9, officeNext10, officeNext11, officeNext12, officeNext13, officeNext14, officeNext15, officeNext16, officeNext17, officeNext18, officeNext19, officeNext20, officeNext21, officeNext22, officeNext23, officeNext24, officeNext25, officeNext26, officeNext27, officeNext28, officeNext29, officeNext30],
    thumbnails: [officeNext1, officeNext2, officeNext3, officeNext4],
  },
  {
    id: 4,
    slug: 'bulvar',
    title: 'Ресторан «BULVAR»',
    location: 'г. Москва',
    date: '2025 г.',
    area: '56 м²',
    studio: 'Impera Studio',
    type: 'Для бизнеса',
    description: 'Ресторан, в интерьер которого невозможно не влюбиться. Здесь одинаково комфортно как для тихого ужина вдвоём, так и для шумной вечеринки с живой музыкой. Изумрудная палитра, теплое золото и мягкая подсветка создают элегантную и утонченную атмосферу. Высокие потолки и светлые стены визуально расширяют пространство. Здесь каждая деталь - от расположения столиков до текстуры обивки, продумана так, чтобы гостям хотелось остаться подольше и вернуться снова.',
    features: [],
    images: [bulvar1, bulvar2, bulvar3, bulvar4, bulvar5, bulvar6, bulvar7, bulvar8, bulvar9, bulvar10, bulvar11, bulvar12, bulvar13],
    thumbnails: [bulvar1, bulvar2, bulvar3, bulvar4],
  },
  {
    id: 5,
    slug: 'beauty-salon',
    title: 'Салон маникюра',
    location: 'г. Люберцы',
    date: '2026 г.',
    area: '93 м²',
    studio: 'Impera Studio',
    type: 'Для бизнеса',
    description:
      'Идеальный баланс между функциональностью и эстетикой «тихой роскоши». Натуральное дерево, мрамор, зелень и изящные латунные элементы подчеркивают высокий статус заведения и формируют премиальный клиентский опыт с первых секунд пребывания в салоне.',
    features: [],
    images: [beautySalon1, beautySalon2, beautySalon3, beautySalon4, beautySalon5, beautySalon6, beautySalon7, beautySalon8, beautySalon9, beautySalon10, beautySalon11, beautySalon12, beautySalon13, beautySalon14, beautySalon15, beautySalon16, beautySalon17, beautySalon18, beautySalon19, beautySalon20, beautySalon21],
    thumbnails: [beautySalon1, beautySalon2, beautySalon3, beautySalon4],
  },
  {
    id: 6,
    slug: 'crazy-brothers',
    title: 'Кафе «Crazy Brothers»',
    location: 'г. Москва',
    date: '2024 г.',
    area: '142 м²',
    studio: 'Impera Studio',
    type: 'Для бизнеса',
    description: 'Мы создали не просто интерьер, а атмосферу, которая стала частью ночной жизни города. Здесь пространство работает на эмоции, вовлекает, удерживает гостей и формирует запоминающийся образ. Мы выстроили концепцию вокруг контраста: тёмные, глубокие оттенки сочетаются с тёплым, мягким светом, а лаконичные формы - с тактильно приятными материалами. Вертикальные световые акценты визуально вытягивают пространство и задают ритм, а живая зелень добавляет ощущение уюта и баланса.',
    features: [],
    images: [crazyBrothers1, crazyBrothers2, crazyBrothers3, crazyBrothers4, crazyBrothers5, crazyBrothers6, crazyBrothers7, crazyBrothers8, crazyBrothers9, crazyBrothers10, crazyBrothers11, crazyBrothers12],
    thumbnails: [crazyBrothers1, crazyBrothers2, crazyBrothers3, crazyBrothers4],
  },
  {
    id: 7,
    slug: 'thai-massage',
    title: 'Салон тайского массажа «Шанти»',
    location: 'г. Электросталь',
    date: '2023 г.',
    area: '500 м²',
    studio: 'Impera Studio',
    description: 'Интерьер, который создаёт ощущение приватности и премиального комфорта, повышая доверие клиентов и их готовность возвращаться. Чёткое зонирование, продуманная навигация и изолированные кабинеты формируют удобный клиентский сценарий и эффективную работу персонала. Тёплая природная палитра, акцентный свет и натуральные материалы усиливают восприятие бренда.',
    features: [],
    images: [taiMassage1, taiMassage2, taiMassage3, taiMassage4, taiMassage5, taiMassage6, taiMassage7, taiMassage8, taiMassage9, taiMassage10, taiMassage11, taiMassage12, taiMassage13, taiMassage14, taiMassage15, taiMassage16, taiMassage17],
    thumbnails: [taiMassage1, taiMassage2, taiMassage3, taiMassage4],
  },
  {
    id: 8,
    slug: 'restaurant-lobnya',
    title: 'Ресторан',
    location: 'Московская область',
    date: '2026 г.',
    area: '200 м²',
    studio: 'Impera Studio',
    type: 'Для бизнеса',
    description:
      'Интерьер, в который хочется возвращаться. Теплая охра, мягкий свет и тактильные поверхности - здесь всё настроено на отдых и неспешное общение. Этот проект доказывает, что современный коммерческий интерьер может быть одновременно функциональным, статусным и по-домашнему уютным.',
    features: [],
    images: [restaurant1, restaurant2, restaurant3, restaurant4, restaurant5, restaurant6, restaurant7, restaurant8, restaurant9, restaurant10, restaurant11],
    thumbnails: [restaurant1, restaurant2, restaurant3, restaurant4],
  },
  {
    id: 9,
    slug: 'hotel-lobnya',
    title: 'Гостиница',
    location: 'г. Лобня',
    date: '2026 г.',
    area: '1200 м²',
    studio: 'Impera Studio',
    type: 'Для бизнеса',
    description:
      'Пространство, где каждая зона - от коридора до лобби - является продолжением единой стилистической истории. Эстетика «тихой роскоши», монохромная палитра, мягкие стеновые панели и обилие текстиля создают атмосферу абсолютного спокойствия.',
    features: [],
    images: [hotelLbn1, hotelLbn2, hotelLbn3, hotelLbn4, hotelLbn5, hotelLbn7, hotelLbn8, hotelLbn9, hotelLbn10, hotelLbn11, hotelLbn12, hotelLbn13, hotelLbn14, hotelLbn15, hotelLbn16, hotelLbn17, hotelLbn18, hotelLbn19, hotelLbn20, hotelLbn21, hotelLbn22, hotelLbn23],
    thumbnails: [hotelLbn1, hotelLbn2, hotelLbn3, hotelLbn4],
  },
  {
    id: 10,
    slug: 'office',
    title: 'Офис компании',
    location: 'г. Москва',
    date: '2025 г.',
    area: '134 м²',
    studio: 'Impera Studio',
    description: 'Офис компании - это не просто столы, стулья и шкафы с документами. Это место, где сотрудники проводят большую часть дня и важно, чтобы они чувствовали себя комфортно. Сдержанная палитра, минимализм, большое количество зелени и естественного света сделали офис стильным и уютным.',
    features: [],
    images: [office1, office2, office3, office4, office5, office6, office7, office8, office9, office10, office11, office12, office13, office14, office15, office16, office17],
    thumbnails: [office1, office2, office3, office4],
  },
  {
    id: 11,
    slug: 'slow-glow',
    title: 'Кальянная «SLOW GLOW»',
    location: 'г. Москва',
    date: '2025 г.',
    area: '171 м²',
    studio: 'Impera Studio',
    description: 'Пространство с атмосферой уединённого премиального отдыха. Темная палитра, бетонные фактуры и тёплый многоуровневый свет формируют глубокий, обволакивающий фон, в котором хочется задержаться. Интерьер зонирован без перегородок - с помощью света, форм и фактур. Это дизайн, который работает на атмосферу, вовлекает гостя и формирует желание возвращаться.',
    features: [],
    images: [slowGlow1, slowGlow2, slowGlow3, slowGlow4, slowGlow5, slowGlow6, slowGlow7, slowGlow8],
    thumbnails: [slowGlow1, slowGlow2, slowGlow3, slowGlow4],
  },
  {
    id: 12,
    slug: 'paratun',
    title: 'Гостиничный комплекс «Паратунь»',
    location: 'г. Петропавловск-Камчатский',
    date: '2024 г.',
    area: '450 м²',
    studio: 'Impera Studio',
    type: 'Для бизнеса',
    description: 'Дизайн гостиничного комплекса создан как комфортная и статусная среда для отдыха и деловых поездок. Номера отличаются продуманной планировкой, спокойной природной палитрой и акцентными материалами, создающими ощущение уюта и приватности на уровне премиального сервиса. Ресторан спроектирован как самостоятельная точка притяжения - место для встреч, мероприятий и гастрономического опыта.',
    features: [],
    images: [paratun1, paratun2, paratun3, paratun4, paratun5, paratun6, paratun7, paratun8, paratun9, paratun10, paratun11, paratun12, paratun13],
    thumbnails: [paratun1, paratun2, paratun3, paratun4],
  },
  {
    id: 13,
    slug: 'antonov-and-company',
    title: 'Антонов и партнеры',
    location: 'г. Москва',
    date: '2026 г.',
    area: '140 м²',
    studio: 'Impera Studio',
    description: 'Современный офис юридической компании, где строгость форм сочетается с ощущением спокойствия и доверия. Лаконичная палитра, натуральные текстуры, продуманное освещение и стеклянные перегородки подчёркивают статус, прозрачность процессов и высокий уровень профессионализма. Пространство выстроено так, чтобы клиент чувствовал уверенность, а команда - комфорт и фокус на работе.',
    features: [],
    images: [
      antonovAndCo1, antonovAndCo2, antonovAndCo3, antonovAndCo4, antonovAndCo5, antonovAndCo6, antonovAndCo7, antonovAndCo8, antonovAndCo9, antonovAndCo10, antonovAndCo11, antonovAndCo12, antonovAndCo13, antonovAndCo14, antonovAndCo15, antonovAndCo16, antonovAndCo17, antonovAndCo18, antonovAndCo19, antonovAndCo20, antonovAndCo21, antonovAndCo22, antonovAndCo23,
    ],
    thumbnails: [antonovAndCo1, antonovAndCo2, antonovAndCo3, antonovAndCo4],
  },
  {
    id: 14,
    slug: 'restaurant',
    title: 'Ресторан «Дворик друзей»',
    location: 'г. Монино',
    date: '2023 г.',
    area: '210 м²',
    studio: 'Impera Studio',
    type: 'Для бизнеса',
    description: 'Этот интерьер создавался как место, где гости могут не просто поесть, а отдохнуть, переключиться и провести время с удовольствием. Здесь каждый найдет формат под своё настроение:\n- комфортные посадочные группы для неспешных бесед,\n- более приватные зоны для уединения,\n- просторный зал для компаний,\n- барная зона и сцена для живых выступлений и событий.',
    features: [],
    images: [dvorik1, dvorik2, dvorik3, dvorik4, dvorik5, dvorik6, dvorik7, dvorik8, dvorik9],
    thumbnails: [dvorik1, dvorik2, dvorik3, dvorik4],
    video: {
      title: 'Реализация проекта',
      sources: [
        { src: '/videos/dvorik-cafe.mp4', type: 'video/mp4' },
        { src: '/videos/dvorik-cafe.webm', type: 'video/webm' }
      ],
      poster: dvorik1
    }
  },
  {
    id: 19,
    slug: 'dental-clinic',
    title: 'Стоматологическая клиника «Лекарь»',
    location: 'г. Подольск',
    date: '2026 г.',
    area: '100 м²',
    studio: 'Impera Studio',
    type: 'Для бизнеса',
    description:
      'Интерьер клиники разработан с акцентом на психологический комфорт пациентов и эффективное использование пространства. Спокойная палитра с пыльно-голубыми и терракотовыми тонами, мягкое освещение и премиальные материалы снимают эмоциональное напряжение, меняя представление о визите к врачу. Эстетичное и функциональное бизнес-решение, создающее высокую лояльность клиентов с первых секунд.',
    features: [],
    images: [dental3, dental1, dental2, dental4, dental5, dental6, dental7, dental8, dental9, dental10, dental11, dental12, dental13, dental14, dental15],
    thumbnails: [dental3, dental1, dental2, dental4],
  },
  {
    id: 15,
    slug: 'kids-room',
    title: 'Детская игровая комната',
    location: 'г. Балашиха',
    date: '2025 г.',
    area: '92 м²',
    studio: 'Impera Studio',
    description: 'Пространство, которое не просто радует маленьких гостей, но и напрямую влияет на прибыль бизнеса. Продуманное зонирование, безопасные материалы, яркие акценты и мебель по росту - делают пребывание детей комфортным и увлекательным. А это больше спокойных родителей, больше проведённого времени на территории и выше средний чек.',
    features: [],
    images: [kidsRoom1, kidsRoom2, kidsRoom3, kidsRoom4, kidsRoom5, kidsRoom6, kidsRoom7, kidsRoom8],
    thumbnails: [kidsRoom1, kidsRoom2, kidsRoom3, kidsRoom4],
  },
  {
    id: 16,
    slug: 'flowers-shop',
    title: 'Салон-магазин цветов',
    location: 'г. Москва',
    date: '2026 г.',
    area: '65 м²',
    studio: 'Impera Studio',
    description: 'Интерьер цветочного салона премиум-класса в Москве построен на контрасте классической архитектуры и современного выставочного пространства с лёгкими венецианскими нотками. Центр композиции - прозрачная витрина с розовым каркасом, где цветы представлены как экспозиция в галерее. Чёрно-белая шахматная плитка, мягкая мебель в розовых оттенках и деликатное освещение формируют стильный и запоминающийся образ.',
    features: [],
    images: [flowersShop1, flowersShop2, flowersShop3, flowersShop4, flowersShop5, flowersShop6, flowersShop7, flowersShop8, flowersShop9, flowersShop10],
    thumbnails: [flowersShop1, flowersShop2, flowersShop3, flowersShop4],
  },
  {
    id: 17,
    slug: 'trophi',
    title: 'Кафе «Trophi»',
    location: 'г. Москва',
    date: '2023 г.',
    area: '90 м²',
    studio: 'Impera Studio',
    description: 'Сюда можно прийти, как на семейный обед, так и на чашечку кофе с друзьями. Пастельная палитра, большое количество света и картины, которые привлекают внимание посетителей - все это делает место узнаваемым и привлекательным для гостей.',
    features: [],
    images: [trophi1, trophi2, trophi3, trophi5, trophi6, trophi7, trophi8, trophi9, trophi11],
    thumbnails: [trophi1, trophi2, trophi3, trophi5],
  },
  {
    id: 18,
    slug: 'hotel',
    title: 'Гостиничный комплекс',
    location: 'г. Монино',
    date: '2023 г.',
    area: '250 м²',
    studio: 'Impera Studio',
    type: 'Для бизнеса',
    description: 'Этот проект - про атмосферу, в которую хочется возвращаться. Мы объединили природные мотивы, тёплые текстуры и современную эстетику, чтобы создать пространство, где гость чувствует не просто комфорт, а настоящее эмоциональное погружение. Графичные панели, фактуры кирпича и дерева, мягкий свет и глубокие оттенки создают ощущение уюта и уединения.',
    features: [],
    images: [hotel1, hotel2, hotel3, hotel4, hotel5, hotel6, hotel7, hotel8, hotel9, hotel10, hotel11, hotel12, hotel13, hotel14, hotel15, hotel16, hotel17, hotel18, hotel19, hotel20, hotel21, hotel22, hotel23, hotel24, hotel25, hotel26],
    thumbnails: [hotel1, hotel2, hotel3, hotel4],
    video: {
      title: 'Реализация проекта',
      sources: [
        { src: '/videos/dvorik-hotel.mp4', type: 'video/mp4' },
        { src: '/videos/dvorik-hotel.webm', type: 'video/webm' }
      ],
      poster: hotel1
    }
  }  
];

